...
async function copyTypescriptLibFiles(outDir: string): Promise<void> {
	try {
		let typescriptLibDir = path.join(import.meta.dirname, '..', '..', 'node_modules', 'typescript', 'lib');
		if (!fs.existsSync(typescriptLibDir)) {
			typescriptLibDir = path.join(import.meta.dirname, '..', 'node_modules', 'typescript', 'lib');
		}
