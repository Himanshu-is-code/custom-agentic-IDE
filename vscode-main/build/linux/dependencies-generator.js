...
import { isDebianArchString } from './debian/types.ts';
...
export async function getDependencies(packageType, buildDir, applicationName, arch) {
    if (packageType === 'deb') {
    ...
    let dependencies;
    if (packageType === 'deb') {
        const chromiumSysroot = await getChromiumSysroot(arch);
        const vscodeSysroot = await getVSCodeSysroot(arch);
        dependencies = generatePackageDepsDebian(files, arch, chromiumSysroot, vscodeSysroot);
    }
    else {
