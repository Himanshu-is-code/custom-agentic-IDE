 ...
 */
async function bundleWithNLS(files, entryPoint, opts) {
 ...
 */
function findColumn(text, needle) {
    const lines = text.split('\n');
...
suite('NLS plugin source maps', () => {
    test('NLS plugin transforms localize calls into placeholders', async () => {
        ...
        const { js, cleanup } = await bundleWithNLS({ 'test/verify.ts': source }, 'test/verify.ts');
        try {
            assert.ok(js.includes('%%NLS:'), 'Bundle should contain %%NLS: placeholder.\nActual JS (first 500 chars):\n' + js.substring(0, 500));
        }
        finally {
            cleanup();
        }
    ...
    test('file without localize calls has correct source map', async () => {
        ...
        const { js, map, cleanup } = await bundleWithNLS({ 'simple.ts': source }, 'simple.ts');
        try {
    ...
    test('sourcesContent should contain original source, not NLS-transformed', async () => {
        ...
        const { mapJson, cleanup } = await bundleWithNLS({ 'test/greeting.ts': source }, 'test/greeting.ts');
        try {
            const sourcesContent = mapJson.sourcesContent ?? [];
            const sources = mapJson.sources ?? [];
            const greetingIdx = sources.findIndex((s) => s.includes('greeting'));
            ...
            assert.ok(!greetingContent.includes('%%NLS:'), 'sourcesContent should NOT contain NLS placeholder.\nActual:\n' + greetingContent);
            assert.ok(greetingContent.includes('localize("myKey", "Hello World")'), 'sourcesContent should contain the exact original localize call.\nActual:\n' + greetingContent);
        }
        finally {
            cleanup();
        }
    ...
    test('NLS-affected nested file keeps a non-duplicated source path', async () => {
        ...
        const { mapJson, cleanup } = await bundleWithNLS({ 'nested/deep/file.ts': source }, 'nested/deep/file.ts');
        try {
            const sources = mapJson.sources ?? [];
            const nestedSource = sources.find((s) => s.endsWith('/nested/deep/file.ts'));
            assert.ok(nestedSource, 'Should find nested/deep/file.ts in sources');
            assert.ok(!nestedSource.includes('/nested/deep/nested/deep/file.ts'), `Source path should not duplicate directory segments. Actual: ${nestedSource}`);
        }
        finally {
            cleanup();
        }
    ...
    test('line mapping correct for code after localize calls', async () => {
        ...
        const { js, map, cleanup } = await bundleWithNLS({ 'test/multi.ts': source }, 'test/multi.ts');
        try {
    ...
    test('column mapping for code on same line after localize call', async () => {
        ...
        const { js, map, cleanup } = await bundleWithNLS({ 'test/coldrift.ts': source }, 'test/coldrift.ts');
        try {
            ...
            assert.ok(columnDrift <= 20, `Column should be close to original. Expected ~${originalCol}, got ${pos.column} (drift: ${columnDrift}). ` +
    ...
    test('class with localize - method positions map correctly', async () => {
        ...
        const { js, map, cleanup } = await bundleWithNLS({ 'test/widget.ts': source }, 'test/widget.ts');
        try {
    ...
    test('many localize calls - line mappings remain correct', async () => {
        ...
        const { js, map, cleanup } = await bundleWithNLS({ 'test/many.ts': source }, 'test/many.ts');
        try {
    ...
    test('post-processed NLS - source map still has original content', async () => {
        ...
        const { js, mapJson, cleanup } = await bundleWithNLS({ 'test/post.ts': source }, 'test/post.ts', { postProcess: true });
        try {
            assert.ok(!js.includes('%%NLS:'), 'JS should not contain NLS placeholders after post-processing');
            const sources = mapJson.sources ?? [];
            const postIdx = sources.findIndex((s) => s.includes('post'));
            ...
            assert.ok(postContent.includes('localize("greeting"'), 'sourcesContent should still contain original localize("greeting") call');
            assert.ok(!postContent.includes('%%NLS:'), 'sourcesContent should not contain NLS placeholders');
        }
        finally {
            cleanup();
        }
    ...
    test('post-processed NLS - column mappings correct after placeholder replacement', async () => {
        ...
        const { js, map, cleanup } = await bundleWithNLS({ 'test/drift.ts': source }, 'test/drift.ts', { postProcess: true });
        try {
            ...
            const originalCol = findColumn(source, '"FINDME"');
            const columnDrift = Math.abs(pos.column - originalCol);
            assert.ok(columnDrift <= 20, `Column drift after NLS post-processing should be small. ` +
    ...
    test('minified bundle with NLS - end-to-end column mapping', async () => {
        ...
        const { js, map, cleanup } = await bundleWithNLS({ 'test/minified.ts': source }, 'test/minified.ts', { postProcess: true, minify: true });
        try {
            assert.ok(!js.includes('%%NLS:'), 'Placeholders should be replaced');
            ...
            assert.strictEqual(pos.line, 9, `Should map "*42" back to line 9. Got line ${pos.line}.`);
        }
        finally {
            cleanup();
        }
