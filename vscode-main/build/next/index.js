...
import { isWebExtension } from '../lib/extensions.ts';
...
function isExtensionHostBundle(filePath) {
 ...
 */
function getEntryPointsForTarget(target) {
    switch (target) {
        case 'desktop':
            return [
                ...workerEntryPoints,
 ...
 */
function getCssBundleEntryPointsForTarget(target) {
    switch (target) {
        case 'desktop':
            return new Set([
                'vs/workbench/workbench.desktop.main',
 ...
 */
function getResourcePatternsForTarget(target) {
    switch (target) {
        case 'desktop':
 ...
 */
function needsBomAdded(filePath) {
...
async function compileStandaloneFiles(outDir, doMinify, target) {
 ...
 */
async function copyAllNonTsFiles(outDir, excludeTests) {
 ...
 */
async function copyResources(outDir, target) {
 ...
 */
function fileContentMapperPlugin(outDir, target) {
    // Cache the replacement strings (computed once)
    let productConfigReplacement;
    let builtinExtensionsReplacement;
    return {
...
async function transpile(outDir, excludeTests) {
    // Find all .ts files
...
// ============================================================================
async function bundle(outDir, doMinify, doNls, doManglePrivates, target, sourceMapBaseUrl) {
 ...
 *--------------------------------------------------------*/`,
    ...
    await Promise.all(allEntryPoints.map(async (entryPoint) => {
        ...
        const plugins = bundleCssEntryPoints.has(entryPoint) ? [] : [cssExternalPlugin()];
    ...
    for (const entry of bootstrapEntryPoints) {
        ...
        const bootstrapPlugins = [inlineMinimistPlugin(), contentMapperPlugin];
        if (doNls) {
    ...
    if (doNls) {
        ...
        const nlsResult = await finalizeNLS(nlsCollector, path.join(REPO_ROOT, outDir), [path.join(REPO_ROOT, 'out-build')]);
        ...
        for (const file of result.outputFiles) {
            ...
            if (file.path.endsWith('.js') || file.path.endsWith('.css')) {
                ...
                if (sourceMapBaseUrl) {
                    ...
                    content = content.replace(/\/\/# sourceMappingURL=.+$/m, `//# sourceMappingURL=${sourceMapBaseUrl}/${relativePath}.map`);
                    content = content.replace(/\/\*# sourceMappingURL=.+\*\/$/m, `/*# sourceMappingURL=${sourceMapBaseUrl}/${relativePath}.map*/`);
                }
...
async function watch() {
    ...
    let pendingTsFiles = new Set();
    let pendingCopyFiles = new Set();
    ...
    try {
        switch (command) {
            case 'transpile':
                ...
                break;
            case 'bundle':
                await bundle(options.out ?? OUT_VSCODE_DIR, options.minify, options.nls, options.manglePrivates, options.target, options.sourceMapBaseUrl);
                break;
            default:
                printUsage();
