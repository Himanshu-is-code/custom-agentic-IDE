...
import { TextModel, analyzeLocalizeCalls, parseLocalizeKeyOrValue } from '../lib/nls-analysis.ts';
/**
 ...
 */
export async function finalizeNLS(collector, outDir, alsoWriteTo) {
    ...
    // Build NLS metadata
    const allMessages = [];
    const moduleToKeys = new Map();
    const moduleToMessages = new Map();
    ...
    await Promise.all(allOutDirs.flatMap(dir => [
        fs.promises.writeFile(path.join(dir, 'nls.messages.json'), JSON.stringify(allMessages)),
        fs.promises.writeFile(path.join(dir, 'nls.keys.json'), JSON.stringify(nlsKeysJson)),
        fs.promises.writeFile(path.join(dir, 'nls.metadata.json'), JSON.stringify(nlsMetadataJson, null, '\t')),
        fs.promises.writeFile(path.join(dir, 'nls.messages.js'), `/*---------------------------------------------------------\n * Copyright (C) Microsoft Corporation. All rights reserved.\n *--------------------------------------------------------*/\nglobalThis._VSCODE_NLS_MESSAGES=${JSON.stringify(allMessages)};`),
    ]));
 ...
 */
export function postProcessNLS(content, indexMap, preserveEnglish) {
    return replaceInOutput(content, indexMap, preserveEnglish);
}
function transformToPlaceholders(source, moduleId) {
    ...
    const allCalls = [...taggedLocalize, ...taggedLocalize2].sort((a, b) => a.call.keySpan.start.line - b.call.keySpan.start.line ||
    ...
    for (const { call, isLocalize2 } of allCalls.reverse()) {
        const keyParsed = parseLocalizeKeyOrValue(call.key);
 ...
 */
function generateNLSSourceMap(originalSource, filePath, edits) {
...
function replaceInOutput(content, indexMap, preserveEnglish) {
...
// Plugin
// ============================================================================
export function nlsPlugin(options) {
