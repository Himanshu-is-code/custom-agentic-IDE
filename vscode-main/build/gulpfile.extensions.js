...
function rewriteTsgoSourceMappingUrlsIfNeeded(build, out, baseUrl) {
    if (!build) {
        return Promise.resolve();
    }
    return util.streamToPromise(gulp.src(path.join(out, '**', '*.js'), { base: out })
 ...
 */
export const compileNonNativeExtensionsBuildTask = task.define('compile-non-native-extensions-build', task.series(bundleMarketplaceExtensionsBuildTask, task.define('bundle-non-native-extensions-build', () => ext.packageNonNativeLocalExtensionsStream(false, false).pipe(gulp.dest('.build')))));
 ...
 */
export const compileAllExtensionsBuildTask = task.define('compile-extensions-build', task.series(cleanExtensionsBuildTask, bundleMarketplaceExtensionsBuildTask, task.define('bundle-extensions-build', () => ext.packageAllLocalExtensionsStream(false, false).pipe(gulp.dest('.build')))));
...
async function buildWebExtensions(isWatch) {
    ...
    const esbuildConfigLocations = await nodeUtil.promisify(glob)(path.join(extensionsPath, '**', 'esbuild.browser.mts'), { ignore: ['**/node_modules'] });
    const promises = [];
