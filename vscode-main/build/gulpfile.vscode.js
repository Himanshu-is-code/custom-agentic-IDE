...
const bundleVSCodeTask = task.define('bundle-vscode', task.series(util.rimraf('out-vscode'), 
...
const minifyVSCodeTask = task.define('minify-vscode', task.series(bundleVSCodeTask, util.rimraf('out-vscode-min'), optimize.minifyTask('out-vscode', `${sourceMappingURLBase}/core`)));
task.task(minifyVSCodeTask);
task.task(task.define('core-ci-old', task.series(task.task('compile-build-with-mangling'), task.parallel(task.task('minify-vscode'), task.task('minify-vscode-reh'), task.task('minify-vscode-reh-web')))));
task.task(task.define('core-ci', task.series(copyCodiconsTask, compileNonNativeExtensionsBuildTask, compileExtensionMediaBuildTask, writeISODate('out-build'), 
...
task.parallel(task.define('esbuild-vscode-min', () => runEsbuildBundle('out-vscode-min', true, true, 'desktop', `${sourceMappingURLBase}/core`)), task.define('esbuild-vscode-reh-min', () => runEsbuildBundle('out-vscode-reh-min', true, true, 'server', `${sourceMappingURLBase}/core`)), task.define('esbuild-vscode-reh-web-min', () => runEsbuildBundle('out-vscode-reh-web-min', true, true, 'server-web', `${sourceMappingURLBase}/core`))))));
/**
...
function packageTask(platform, arch, sourceFolderName, destinationFolderName, _opts) {
...
function hasAuthenticodeSignature(filePath) {
...
async function stripAuthenticodeSignature(filePath) {
...
function prepareCopilotRipgrepShimTask(platform, arch, destinationFolderName) {
...
BUILD_TARGETS.forEach(buildTarget => {
    ...
    const [vscode, vscodeMin] = ['', 'min'].map(minified => {
        ...
        if (useEsbuildTranspile) {
            const esbuildBundleTask = task.define(`esbuild-bundle${dashed(platform)}${dashed(arch)}${dashed(minified)}`, () => runEsbuildBundle(sourceFolderName, !!minified, true, 'desktop', minified && useCdnSourceMapsForPackagingTasks ? `${sourceMappingURLBase}/core` : undefined));
            vscodeTask = task.define(`vscode${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(copyCodiconsTask, cleanExtensionsBuildTask, compileNonNativeExtensionsBuildTask, compileCopilotExtensionBuildTask, compileExtensionMediaBuildTask, writeISODate('out-build'), esbuildBundleTask, vscodeTaskCI));
        }
        else {
            vscodeTask = task.define(`vscode${dashed(platform)}${dashed(arch)}${dashed(minified)}`, task.series(minified ? compileBuildWithManglingTask : compileBuildWithoutManglingTask, cleanExtensionsBuildTask, compileNonNativeExtensionsBuildTask, compileCopilotExtensionBuildTask, compileExtensionMediaBuildTask, minified ? minifyVSCodeTask : bundleVSCodeTask, vscodeTaskCI));
        }
...
// #region nls
task.task(task.define('vscode-translations-export', task.series(task.task('core-ci'), compileAllExtensionsBuildTask, function () {
    const pathToMetadata = './out-build/nls.metadata.json';
    const pathToExtensions = '.build/extensions/*';
    const pathToSetup = 'build/win32/i18n/messages.en.isl';
    return es.merge(gulp.src(pathToMetadata).pipe(i18n.createXlfFilesForCoreBundle()), gulp.src(pathToSetup).pipe(i18n.createXlfFilesForIsl()), gulp.src(pathToExtensions).pipe(i18n.createXlfFilesForExtensions())).pipe(vfs.dest('../vscode-translations-export'));
})));
