...
function isNLSKeysFormat(value) {
    if (value === undefined) {
        return false;
    }
    const candidate = value;
...
const editorProject = 'vscode-editor', workbenchProject = 'vscode-workbench', extensionsProject = 'vscode-extensions', setupProject = 'vscode-setup', serverProject = 'vscode-server', sessionsProject = 'vscode-sessions';
...
export function createXlfFilesForExtensions() {
    ...
    return eventStream.through(function (extensionFolder) {
        ...
        eventStream.merge(gulp.src([`.build/extensions/${extensionFolderName}/package.nls.json`, `.build/extensions/${extensionFolderName}/**/nls.metadata.json`], { allowEmpty: true }), createL10nBundleForExtension(extensionFolderName, EXTERNAL_EXTENSIONS.includes(extensionId))).pipe(eventStream.through(function (file) {
            if (file.isBuffer()) {
...
export function prepareI18nPackFiles(resultingTranslationPaths) {
    ...
    const errors = [];
    return eventStream.through(function (xlf) {
...
export function prepareIslFiles(language) {
    const parsePromises = [];
    return eventStream.through(function (xlf) {
        const stream = this;
...
function createIslFile(name, messages, language) {
    const content = [];
    let originalContent;
