...
suite('Policy E2E conversion', () => {
    ...
    test('should render macOS manifest from policies list', async () => {
        ...
        assert.strictEqual(normalizeTimestamp(enUsManifest.contents), normalizeTimestamp(expectedContent), 'macOS manifest should match the fixture (ignoring timestamp)');
    });
    ...
    test('should render macOS manifest with fr-fr locale', async () => {
        ...
        // Compare the rendered manifest with the fixture, ignoring the timestamp
        const normalizeTimestamp = (content) => content.replace(/<date>.*?<\/date>/, '<date>TIMESTAMP</date>');
        assert.strictEqual(normalizeTimestamp(frFrManifest.contents), normalizeTimestamp(expectedContent), 'macOS fr-fr manifest should match the fixture (ignoring timestamp)');
    });
    ...
    test('should successfully parse the checked-in policyData.jsonc', async () => {
        ...
        const errors = [];
        const policyData = JSONC.parse(raw, errors);
