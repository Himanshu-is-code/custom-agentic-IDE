...
export function bundleTask(opts) {
    return function () {
...
export function minifyTask(src, sourceMapBaseUrl) {
    ...
    return cb => {
        ...
        pump(gulp.src([src + '/**', '!' + src + '/**/*.map']), esbuildFilter, sourcemaps.init({ loadMaps: true }), es.map((f, cb) => {
            esbuild.build({
                entryPoints: [f.path],
                minify: true,
                sourcemap: 'external',
        ...
        }), esbuildFilter.restore, svgFilter, svgmin(), svgFilter.restore, sourcemaps.write('./', {
            sourceMappingURL,
            sourceRoot: undefined,
            includeContent: true,
            addComment: true
