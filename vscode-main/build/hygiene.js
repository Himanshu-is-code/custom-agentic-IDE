 ...
 */
export function checkCopilotEnginesVersion(repoRoot) {
...
export function hygiene(some, runEslint = true) {
    ...
    const unicode = es.through(function (file) {
        ...
        lines.forEach((line, i) => {
            ...
            if (m) {
                console.error(file.relative + `(${i + 1},${m.index + 1}): Unexpected unicode character: "${m[0]}" (charCode: ${m[0].charCodeAt(0)}). To suppress, use // allow-any-unicode-next-line`);
                errorCount++;
            }
        });
        this.emit('data', file);
    ...
    streams.push(result.pipe(filter(Array.from(stylelintFilter))).pipe(gulpstylelint(((message, isError) => {
        if (isError) {
    ...
    let count = 0;
    return es.merge(...streams).pipe(es.through(function (data) {
        count++;
        ...
        process.stdout.write('\n');
        if (errorCount > 0) {
            this.emit('error', 'Hygiene failed with ' +
                errorCount +
...
function createGitIndexVinyls(paths) {
    ...
    const fns = paths.map((relativePath) => () => new Promise((c, e) => {
        ...
        fs.stat(fullPath, (err, stat) => {
            ...
            cp.exec(process.platform === 'win32' ? `git show :${relativePath}` : `git show ':${relativePath}'`, { maxBuffer: Math.max(stat.size * 2, 1024 * 1024), encoding: 'buffer' }, (err, out) => {
                if (err) {
                    return e(err);
                }
                c(new VinylFile({
        ...
        cp.exec('git diff --cached --name-only', { maxBuffer: 2000 * 1024 }, (err, out) => {
            ...
            if (some.length > 0) {
                ...
                createGitIndexVinyls(some)
                    .then((vinyls) => {
                    return new Promise((c, e) => hygiene(es.readArray(vinyls).pipe(filter(Array.from(all))))
                        .on('end', () => c())
                        .on('error', e));
                })
                    .catch((err) => {
