...
async function npmInstallAsync(dir, opts) {
    const finalOpts = {
        env: { ...process.env },
        ...(opts ?? {}),
...
function getNpmrcConfigKeys(npmrcPath) {
...
function clearInheritedNpmrcConfig(dir, env) {
...
function ensureAgentHarnessLink(sourceRelativePath, linkPath) {
    ...
    try {
        ...
        if (process.platform === 'win32' && !isDirectory && error.code === 'EPERM') {
