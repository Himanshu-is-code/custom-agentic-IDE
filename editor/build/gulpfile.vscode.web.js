...
function runEsbuildBundle(outDir, minify, nls, sourceMapBaseUrl) {
...
const bundleVSCodeWebTask = task.define('bundle-vscode-web-OLD', task.series(util.rimraf('out-vscode-web'), optimize.bundleTask({
    out: 'out-vscode-web',
    esm: {
        src: 'out-build',
...
const minifyVSCodeWebTask = task.define('minify-vscode-web-OLD', task.series(bundleVSCodeWebTask, util.rimraf('out-vscode-web-min'), optimize.minifyTask('out-vscode-web', `https://main.vscode-cdn.net/sourcemaps/${commit}/core`)));
...
function packageTask(sourceFolderName, destinationFolderName) {
    ...
    return () => {
        ...
        const pwaicons = es.merge(gulp.src('resources/server/code-192.png', { base: 'resources/server' }), gulp.src('resources/server/code-512.png', { base: 'resources/server' }));
        const all = es.merge(packageJsonStream, license, sources, deps, favicon, manifest, pwaicons);
        const result = all
...
['', 'min'].forEach(minified => {
    ...
    const vscodeWebTaskCI = task.define(`vscode-web${dashed(minified)}-ci`, task.series(copyCodiconsTask, compileWebExtensionsBuildTask, minified ? esbuildBundleVSCodeWebMinTask : esbuildBundleVSCodeWebTask, util.rimraf(path.join(BUILD_ROOT, destinationFolderName)), packageTask(sourceFolderName, destinationFolderName)));
    task.task(vscodeWebTaskCI);
    const vscodeWebTask = task.define(`vscode-web${dashed(minified)}`, task.series(compileBuildWithManglingTask, vscodeWebTaskCI));
