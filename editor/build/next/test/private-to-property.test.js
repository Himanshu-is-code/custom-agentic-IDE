...
suite('convertPrivateFields', () => {
    ...
    test('static async private method — no token fusion', async () => {
        ...
        const result = convertPrivateFields(code, 'test.js');
        assert.doesNotThrow(() => new Function(result.code), 'static async private method must produce valid JS, got:\n' + result.code);
        const exec = new Function(`
			${result.code}
			return Foo.go();
		`);
    ...
    test('heritage clause — extends expression resolves outer private field, not inner', () => {
        ...
        assert.ok(result.code.includes('this.$a, Object'), 'heritage clause should reference outer replacement ($a), got:\n' + result.code);
    });
    test('heritage clause runtime — extends uses correct outer private field', () => {
        ...
        assert.strictEqual(new Function(result.code)(), true, 'inner class should extend Base via outer private field, code:\n' + result.code);
    });
    test('generated name must not collide with existing public property', () => {
        ...
        assert.ok(!fieldDecls || fieldDecls.length <= 1, 'should not produce duplicate $a property declarations, got:\n' + result.code);
    });
    test('collision with existing property — runtime correctness', () => {
        ...
        assert.strictEqual(new Function(result.code)(), 'public,private', 'public and private properties must remain distinct, code:\n' + result.code);
    });
    test('collision avoidance — string-literal public property name', () => {
        ...
        const result = convertPrivateFields(code, 'test.js');
        assert.strictEqual(new Function(result.code)(), 'public,private', 'string-literal public property must not collide, code:\n' + result.code);
    });
    test('collision avoidance — computed string-literal public property name', () => {
        ...
        const result = convertPrivateFields(code, 'test.js');
        assert.strictEqual(new Function(result.code)(), 'public,private', 'computed string-literal public property must not collide, code:\n' + result.code);
    });
    test('brand check in heritage clause resolves to outer scope', () => {
        ...
        // not Inner.#brand ($b)
        assert.ok(result.code.includes('\'$a\' in obj'), 'brand check in heritage clause should use outer replacement, got:\n' + result.code);
    });
});
suite('adjustSourceMap', () => {
     ...
     */
    function createIdentitySourceMap(code, sourceName) {
