 ...
 */
export function convertPrivateFields(code, filename) {
    const t1 = Date.now();
    ...
    function createWalkInClass(classNode) {
        return function walkInClass(child) {
    ...
    function resolvePrivateName(name) {
 ...
 */
export function adjustSourceMap(sourceMapJson, originalCode, edits) {
    if (edits.length === 0) {
    ...
    const editStarts = new Array(n);
    const editEnds = new Array(n);
    const cumShifts = new Array(n); // cumulative shift *after* edit[i]
    let cumShift = 0;
    for (let i = 0; i < n; i++) {
 ...
 */
function buildLineStartsAfterEdits(originalCode, edits) {
    const starts = [0];
    let oldPos = 0;
    let newPos = 0;
    for (const edit of edits) {
