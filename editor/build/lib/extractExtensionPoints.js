 ...
 */
export function extractExtensionPointNamesFromFile(sourceFile) {
    const results = [];
    visit(sourceFile);
    return results;
    function visit(node) {
    ...
    function handleRegisterCall(call) {
    ...
    function handleInlineDescriptor(call, obj) {
    ...
    function handleParameterReference(registerCall, paramName) {
        // Walk up to find the containing function
        let current = registerCall.parent;
        ...
        const paramIndex = fn.parameters.findIndex(p => ts.isIdentifier(p.name) && p.name.text === paramName);
        if (paramIndex < 0) {
            return;
        }
    ...
    function handleImportedDescriptor(identifier) {
        ...
        for (const stmt of sourceFile.statements) {
            ...
            for (const element of stmt.importClause.namedBindings.elements) {
                ...
                const resolvedPath = path.resolve(path.dirname(sourceFile.fileName), modulePath.replace(/\.js$/, '.ts'));
                try {
...
function findExtensionPointProperty(obj) {
...
function findExtensionPointInVariable(sourceFile, varName) {
    for (const stmt of sourceFile.statements) {
