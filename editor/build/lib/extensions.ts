...
function fromLocalEsbuild(extensionPath: string, esbuildConfigFileName: string): Stream {
	...
	new Promise<void>((resolve, reject) => {
		const args = (esbuildScript.endsWith('.mts') || esbuildScript.endsWith('.ts'))
			? ['--import', 'tsx', esbuildScript]
			: [esbuildScript];
		const proc = cp.execFile(process.argv[0], args, { cwd: extensionPath }, (error, _stdout, stderr) => {
			if (error) {
				return reject(error);
			}
