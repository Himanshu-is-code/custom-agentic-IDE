...
import { analyzeLocalizeCalls, TextModel, parseLocalizeKeyOrValue } from './nls-analysis.ts';
/**
...
export function nls(options) {
    ...
    const output = input
        ...
        .pipe(eventStream.through(function (f) {
        if (!f.sourceMap) {
...
const _nls = (() => {
    ...
    function patch(typescript, javascript, sourcemap, options) {
    ...
    function patchFile(javascriptFile, typescript, options) {
        ...
        const moduleId = javascriptFile.relative
            .replace(/\.js$/, '')
            .replace(/\\/g, '/');
        const { javascript, sourcemap, nlsKeys, nlsMessages } = patch(typescript, javascriptFile.contents.toString(), javascriptFile.sourceMap, options);
