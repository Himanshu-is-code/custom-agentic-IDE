...
export function spawnTsgo(projectPath, config, onComplete) {
    function runReporter(output) {
    ...
    return new Promise((resolve, reject) => {
        child.on('exit', code => {
            ...
            if (code === 0) {
                ...
                fancyLog(ansiColors.yellow(`tsgo exited with code ${code ?? 'unknown'} (ignored during build)`));
                Promise.resolve(onComplete?.()).then(() => resolve(), reject);
            }
        });
        child.on('error', err => {
...
export function createTsgoStream(projectPath, config, onComplete) {
