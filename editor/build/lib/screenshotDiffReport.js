...
class ImageDiffResult {
    changedPixelCount;
    maxChannelDelta;
    constructor(changedPixelCount, maxChannelDelta) {
...
async function computePixelDiffs(changed, serviceUrl, localManifestDir) {
...
function shouldIncludeInReport(labels) {
    return !labels?.some(l => EXCLUDED_LABELS.has(l));
}
function collectErrored(local) {
    const errored = [];
...
function diffManifests(local, base) {
...
function loadImageUrl(serviceUrl, hash) {
    return `${serviceUrl.replace(/\/$/, '')}/images/${hash}`;
}
function generateMarkdown(diff, serviceUrl, baseSha, currentSha, pixelDiffs) {
...
async function main() {
    ...
    fs.writeFileSync(path.join(tmpDir, 'screenshotDiffReport.json'), JSON.stringify(diff, null, 2));
