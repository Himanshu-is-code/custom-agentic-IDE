...
function fromLocal(extensionPath, forWeb, _disableMangle) {
    ...
    if (hasEsbuild) {
        ...
        input = isStandardEsbuild
            ? es.merge(fromLocalEsbuild(extensionPath, esbuildConfigFileName), 
...
export function typeCheckExtension(extensionPath, forWeb) {
...
export function typeCheckExtensionStream(extensionPath, forWeb) {
    const tsconfigFileName = forWeb ? 'tsconfig.browser.json' : 'tsconfig.json';
    const tsconfigPath = path.join(extensionPath, tsconfigFileName);
...
function fromLocalEsbuild(extensionPath, esbuildConfigFileName) {
    ...
    new Promise((resolve, reject) => {
        ...
        if (packagedDependencies.length > 0) {
            const packagedDependencyFileNames = packagedDependencies.flatMap(dependency => glob.sync(path.join(extensionPath, 'node_modules', dependency, '**'), { nodir: true, dot: true })
...
export function fromVsix(vsixPath, { name: extensionName, version, sha256, metadata }) {
 ...
 * @returns a stream
 */
export function packageNonNativeLocalExtensionsStream(forWeb, disableMangle) {
 ...
 * @returns a stream
 */
export function packageNativeLocalExtensionsStream(forWeb, disableMangle) {
 ...
 * @returns a stream
 */
export function packageAllLocalExtensionsStream(forWeb, disableMangle) {
    return es.merge([
 ...
 */
export function packageCopilotExtensionStream(disableMangle) {
    const extensionPath = path.join(root, 'extensions', 'copilot');
    if (!fs.existsSync(extensionPath)) {
        return es.readArray([]);
    }
    const localExtensionsStream = minifyExtensionResources(fromLocal(extensionPath, false, disableMangle)
    ...
    const dependenciesSrc = productionDependencies.map(d => path.relative(root, d)).map(d => [`${d}/**`, `!${d}/**/{test,tests}/**`]).flat();
    return es.merge(localExtensionsStream, gulp.src(dependenciesSrc, { base: '.' })
        .pipe(util2.cleanNodeModules(path.join(root, 'build', '.moduleignore')))
        .pipe(util2.cleanNodeModules(path.join(root, 'build', `.moduleignore.${process.platform}`)))).pipe(util2.setExecutableBit(['**/*.sh']));
}
...
export async function esbuildExtensions(taskName, isWatch, scripts) {
...
export function buildExtensionMedia(isWatch, outputRoot) {
...
export function getBuildRootsForExtension(extensionPath) {
