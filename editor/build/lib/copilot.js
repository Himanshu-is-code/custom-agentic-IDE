 ...
 */
function toNodePlatformArch(platform, arch) {
 ...
 */
export function getRipgrepExcludeFilter(platform, arch) {
 ...
 */
export function getCopilotExcludeFilter(platform, arch) {
    const { nodePlatform, nodeArch } = toNodePlatformArch(platform, arch);
 ...
 * prebuilds layout.
 */
export function getCopilotRuntimePrebuildFiles(platform, arch, nodeModulesRoot = 'node_modules') {
    const { nodePlatform, nodeArch } = toNodePlatformArch(platform, arch);
    const targetPlatformArch = `${nodePlatform}-${nodeArch}`;
 ...
 */
export function prepareBuiltInCopilotRipgrepShim(platform, arch, builtInCopilotExtensionDir, appNodeModulesDir) {
    const { nodePlatform, nodeArch } = toNodePlatformArch(platform, arch);
...
function pruneNonTargetCopilotSdkPrebuilds(targetPlatformArch, copilotSdkBase) {
