...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	render(parent: HTMLElement): void {
		...
		this.initTimelineNavigation(this.container);
	...
	private updateActiveTimelineItem(forcedIndex?: number): void {
		...
		if (typeof forcedIndex === 'number') {
			...
			const requestElements = Array.from(this.listContainer.querySelectorAll('.interactive-request'));
			const containerRect = this.listContainer.getBoundingClientRect();
			
			// Find the last request element whose top is above the bottom of the viewport
			let lastVisibleRequestEl: Element | undefined;
			for (const el of requestElements) {
				const rect = el.getBoundingClientRect();
				if (rect.top - containerRect.top < containerRect.height - 20) {
					lastVisibleRequestEl = el;
				} else {
					break;
				}
			}

			if (lastVisibleRequestEl) {
				const elText = lastVisibleRequestEl.textContent ?? '';
				const matchIdx = requests.findIndex(req => elText.includes(req.messageText) || req.messageText.includes(elText));
				if (matchIdx !== -1) {
					activeIndex = matchIdx;
				}
			}

			// If we are at the bottom, force the last request to be active
			const isAtBottom = this.listWidget.scrollTop + this.listWidget.renderHeight >= this.listWidget.scrollHeight - 20;
			if (isAtBottom) {
				activeIndex = requests.length - 1;
			}
		}
